...
```markdown
#准备ai脚本(ai生成的内容)：
   1. 这一部分内容需要您请求大语音模型生成
   2. 我会准备一个示例文稿（关于mygo!!!!!的剧情解析）用于示例
   3. 不管是搭建知识库也好，直接喂进大语音模型也行，甚至是您自己微调的模型，都需要使用提示词，让大语音模型按照格式输出
   4. 在`ai提示词.md`中有相应的提示词，分别用于系统提示词，提示词与指导ai使用视频字幕生成片段理解的提示词
```
